...
type alias CurrentBuild =
    ...
    , output : Maybe BetaBuildOutput.Model
    }
...
type Msg
    ...
    | BetaBuildOutputMsg Int BetaBuildOutput.Msg
...
subscriptions model =
    Sub.batch
        ...
        , case model.currentBuild |> RemoteData.toMaybe |> Maybe.andThen .output of
            ...
            Just buildOutput ->
                Sub.map (BetaBuildOutputMsg model.browsingIndex) buildOutput.events
...
update action model =
    case action of
        ...
        BetaBuildOutputMsg browsingIndex action ->
                        ...
                        ( Just currentBuild, Just output ) ->
                            let
                                ( newOutput, cmd, outMsg ) =
                                    BetaBuildOutput.update action output
                            ...
                            in
                                ...
                                , Cmd.batch
                                    [ newCmd
                                    , Cmd.map (BetaBuildOutputMsg browsingIndex) cmd
                                    ]
                                )

                        _ ->
                            Debug.crash "impossible (received action for missing BetaBuildOutput)"
            else
                ( model, Cmd.none )
            ...
            ( newModel, cmd ) =
                if build.status == Concourse.BuildStatusPending then
                    ...
                    case model.currentBuild |> RemoteData.toMaybe |> Maybe.andThen .prep of
                        Nothing ->
                            initBetaBuildOutput build withBuild
...
initBetaBuildOutput : Concourse.Build -> Model -> ( Model, Cmd Msg )
initBetaBuildOutput build model =
    let
        ( output, outputCmd ) =
            BetaBuildOutput.init { hash = model.hash } build
    in
        ( { model
            | currentBuild =
                RemoteData.map
        ...
        , Cmd.map (BetaBuildOutputMsg model.browsingIndex) outputCmd
        )
        ...
        Just currentBuild ->
            Html.div [ class "with-fixed-header" ]
                ...
                , Html.div [ class "scrollable-body build-body" ] <|
                    ...
                    , Html.Lazy.lazy2 viewBetaBuildOutput model.browsingIndex <|
                        currentBuild.output
                    , Html.div
                        [ classList
                            [ ( "keyboard-help", True )
...
viewBetaBuildOutput : Int -> Maybe BetaBuildOutput.Model -> Html Msg
viewBetaBuildOutput browsingIndex output =
    case output of
        Just o ->
            Html.map (BetaBuildOutputMsg browsingIndex) (BetaBuildOutput.view o)
...
handleOutMsg : BetaBuildOutput.OutMsg -> Model -> ( Model, Cmd Msg )
handleOutMsg outMsg model =
    case outMsg of
        BetaBuildOutput.OutNoop ->
            ( model, Cmd.none )

        BetaBuildOutput.OutBuildStatus status date ->
            case model.currentBuild |> RemoteData.toMaybe of
                Nothing ->
                    ( model, Cmd.none )
