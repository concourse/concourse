...
var _ = Describe("RenamePipeline", func() {
	...
	Context("when not specifying a new name", func() {
		It("fails and says you should provide a new name for the pipeline", func() {
			flyCmd := exec.Command(flyPath, "-t", targetName, "rename-pipeline", "-p", "some-pipeline")
			...
			Expect(sess.Err).To(gbytes.Say("error: the required flag `" + osFlag("n", "name") + "' was not specified"))
		})
	})

	Context("when all the inputs are provided", func() {
		It("successfully renames the pipeline to the provided name", func() {
			flyCmd := exec.Command(flyPath, "-t", targetName, "rename-pipeline", "-p", "some-pipeline", "-n", newName)
		...
		Context("when the pipeline is not found", func() {
			BeforeEach(func() {
				atcServer.SetHandler(1, ghttp.RespondWith(http.StatusNotFound, ""))
			})
			...
			It("returns false and no error", func() {
				...
				Eventually(sess).Should(gexec.Exit(1))
				Expect(atcServer.ReceivedRequests()).To(HaveLen(2))
				Expect(sess.Err).To(gbytes.Say(fmt.Sprintf("pipeline %s not found", "some-pipeline")))
			})
		})

		Context("when an error occurs", func() {
			BeforeEach(func() {
				atcServer.SetHandler(1, ghttp.RespondWith(http.StatusTeapot, ""))
			})
