...
var _ = Describe("PipelineDB", func() {
	...
	Describe("build preparation", func() {
		...
		Context("unpaused pipeline", func() {
			BeforeEach(func() {
				var err error
				build, err = pipelineDB.CreateJobBuild(jobConfig.Name)
				Expect(err).NotTo(HaveOccurred())
				...
				var scheduled bool
				scheduled, err = pipelineDB.ScheduleBuild(build.ID, jobConfig)
				Expect(err).NotTo(HaveOccurred())
		...
		Context("first time scheduling build", func() {
			JustBeforeEach(func() {
				var err error
				build, err = pipelineDB.CreateJobBuild(jobConfig.Name)
				Expect(err).NotTo(HaveOccurred())
	...
	Describe("Jobs", func() {
		...
		Describe("GetRunningBuildsBySerialGroup", func() {
			...
			BeforeEach(func() {
				...
				scheduled, err := pipelineDB.ScheduleBuild(scheduledBuild.ID, atc.JobConfig{Name: "some-job"})
				Expect(err).NotTo(HaveOccurred())
				Expect(scheduled).To(BeTrue())
			...
			It("returns a list of builds the matches the job passed in that are started or scheduled and have a different serial group", func() {
				builds, err := pipelineDB.GetRunningBuildsBySerialGroup("some-job", []string{"matching-job"})
				Expect(err).NotTo(HaveOccurred())
