...
func AddUniqueIndexesToMaterializedViews(tx migration.LimitedTx) error {
	...
	_, err = tx.Exec(`
		CREATE UNIQUE INDEX next_builds_per_job _id ON next_builds_per_job (id)
	`)
	if err != nil {
		return err
	}
	...
	_, err = tx.Exec(`
		CREATE UNIQUE INDEX transition_builds_per_job _id ON transition_builds_per_job (id)
	`)
	if err != nil {
		return err
	}
