...
func CleanUpContainerColumns(tx migration.LimitedTx) error {
	_, err := tx.Exec(`
		...
		ADD COLUMN meta_resource_id integer NOT NULL DEFAULT 0,
		ADD COLUMN meta_resource_type_id integer NOT NULL DEFAULT 0,
		...
		ADD COLUMN meta_resource_name text NOT NULL DEFAULT '',
		ADD COLUMN meta_resource_type_name text NOT NULL DEFAULT '',
		DROP COLUMN type,
