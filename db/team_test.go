...
var _ = Describe("Team", func() {
	...
	Describe("FindContainerByHandle", func() {
		...
		BeforeEach(func() {
			...
			creatingContainer, err := defaultTeam.CreateBuildContainer(defaultWorker.Name(), build.ID(), atc.PlanID("some-job"), db.ContainerMetadata{Type: "task", StepName: "some-task"})
			Expect(err).NotTo(HaveOccurred())
	...
	Describe("FindWorkerForResourceCheckContainer", func() {
		var resourceConfig *db.UsedResourceConfig
		...
		Context("when there is a creating container", func() {
			BeforeEach(func() {
				_, err := defaultTeam.CreateResourceCheckContainer(defaultWorker.Name(), resourceConfig, db.ContainerMetadata{Type: "check"})
				Expect(err).NotTo(HaveOccurred())
			})
		...
		Context("when there is a created container", func() {
			var originalCreatedContainer db.CreatedContainer

			BeforeEach(func() {
				creatingContainer, err := defaultTeam.CreateResourceCheckContainer(defaultWorker.Name(), resourceConfig, db.ContainerMetadata{Type: "check"})
				Expect(err).NotTo(HaveOccurred())
