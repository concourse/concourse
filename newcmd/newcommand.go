...
func InitializeFlags(w *cobra.Command) {
	w.Flags().IP("bind-ip", net.IP{0, 0, 0, 0}, "IP address on which to listen for web traffic.")
	w.Flags().String("bind-port", "8080", "Port on which to listen for HTTP traffic.")

	w.Flags().Uint16("tls-bind-port", 0, "Port on which to listen for HTTPS traffic.")
	w.Flags().String("tls-cert", "", "File containing an SSL certificate.")
	w.Flags().String("tls-key", "", "File containing an RSA private key, used to encrypt HTTPS traffic.")

	w.Flags().Bool("enable-lets-encrypt", false, "Automatically configure TLS certificates via Let's Encrypt/ACME.")
	w.Flags().String("lets-encrypt-acme-url", "https://acme-v02.api.letsencrypt.org/directory", "URL of the ACME CA directory endpoint.")

	w.Flags().String("external-url", "", "URL used to reach any ATC from the outside world.")

	// Postgres configuration flags
	w.Flags().String("postgres-host", "127.0.0.1", "The host to connect to.")
	w.Flags().Uint16("postgres-port", 5432, "The port to connect to.")
	w.Flags().String("postgres-socket", "", "Path to a UNIX domain socket to connect to.")
	w.Flags().String("postgres-user", "", "The user to sign in as.")
	w.Flags().String("postgres-password", "", "The user's password.")
	w.Flags().String("postgres-sslmode", "disable", "Whether or not to use SSL.")
	w.Flags().String("postgres-ca-cert", "", "CA cert file location, to verify when connecting with SSL.")
	w.Flags().String("postgres-client-cert", "", "Client cert file location.")
	w.Flags().String("postgres-client-key", "", "Client key file location.")
	w.Flags().Duration("postgres-connect-timeout", 5*time.Minute, "Dialing timeout. (0 means wait indefinitely)")
	w.Flags().String("postgres-database", "atc", "The name of the database to use.")

	w.Flags().StringToInt("concurrent-request-limit", nil, "Limit the number of concurrent requests to an API endpoint (Example: ListAllJobs:5)")

	w.Flags().Int("api-max-conns", 10, "The maximum number of open connections for the api connection pool.")
	w.Flags().Int("backend-max-conns", 50, "The maximum number of open connections for the backend connection pool.")

	// TODO: CredentialManagers   creds.Managers

	w.Flags().String("encryption-key", "", "A 16 or 32 length key used to encrypt sensitive information before storing it in the database.")
	w.Flags().String("old-encryption-key", "", "Encryption key previously used for encrypting sensitive information. If provided without a new key, data is encrypted. If provided with a new key, data is re-encrypted.")

	w.Flags().IP("debug-bind-ip", net.IP{127, 0, 0, 1}, "IP address on which to listen for the pprof debugger endpoints.")
	w.Flags().Uint16("debug-bind-port", 8079, "Port on which to listen for the pprof debugger endpoints.")

	w.Flags().Duration("intercept-idle-timeout", 0*time.Minute, "Length of time for a intercepted session to be idle before terminating.")

	w.Flags().Bool("enable-global-resources", false, "Enable equivalent resources across pipelines and teams to share a single version history.")

	w.Flags().Duration("component-runner-interval", 10*time.Second, "Interval on which runners are kicked off for builds, locks, scans, and checks")

	w.Flags().Duration("lidar-scanner-interval", 10*time.Second, "Interval on which the resource scanner will run to see if new checks need to be scheduled")
	w.Flags().Duration("lidar-checker-interval", 10*time.Second, "Interval on which the resource checker runs any scheduled checks")

	w.Flags().Duration("global-resource-check-timeout", 1*time.Hour, "Time limit on checking for new versions of resources.")
	w.Flags().Duration("resource-checking-interval", 1*time.Minute, "Interval on which to check for new versions of resources.")
	w.Flags().Duration("resource-with-webhook-checking-interval", 1*time.Minute, "Interval on which to check for new versions of resources that has webhook defined.")
	w.Flags().Int("max-checks-per-second", 0, "Maximum number of checks that can be started per second. If not specified, this will be calculated as (# of resources)/(resource checking interval). -1 value will remove this maximum limit of checks per second.")

	creds.InitializeSecretRetryFlags(w)
	creds.InitializeCachedSecretsFlags(w)

	viper.BindPFlags(w.Flags())
	viper.BindPFlag("lets_encrypt.enable", w.Flags().Lookup("enable-lets-encrypt"))

	// Bind postgres configuration keys to flags
	viper.BindPFlag("postgres.host", w.Flags().Lookup("postgres-host"))
	viper.BindPFlag("postgres.port", w.Flags().Lookup("postgres-port"))
	viper.BindPFlag("postgres.socket", w.Flags().Lookup("postgres-socket"))
	viper.BindPFlag("postgres.user", w.Flags().Lookup("postgres-user"))
	viper.BindPFlag("postgres.password", w.Flags().Lookup("postgres-password"))
	viper.BindPFlag("postgres.sslmode", w.Flags().Lookup("postgres-sslmode"))
	viper.BindPFlag("postgres.ca_cert", w.Flags().Lookup("postgres-ca-cert"))
	viper.BindPFlag("postgres.client_cert", w.Flags().Lookup("postgres-client-cert"))
	viper.BindPFlag("postgres.client_key", w.Flags().Lookup("postgres-client-key"))
	viper.BindPFlag("postgres.connect_timeout", w.Flags().Lookup("postgres-connect-timeout"))
	viper.BindPFlag("postgres.database", w.Flags().Lookup("postgres-database"))
...
func (c *Potatoes) GetCiper(field string) (ciper.AEAD, error) {
...
func (v *Potatoes) GetPort(field string) (uint16, error) {
...
func (c *Potatoes) GetIP(field string) (net.IP, error) {
	value := viper.GetString(field)
	if value == "" {
		return nil, nil
	}
	...
	if parsedIP == nil {
		return nil, fmt.Errorf("%s - invalid IP '%s'", field, value)
	}

	return parsedIP, nil
}

func (c *Potatoes) GetFile(field string) (string, error) {
	value := viper.GetString(field)
	if value == "" {
		return "", nil
	}
	...
	if stat.IsDir() {
		return "", fmt.Errorf("%s - path '%s' is a directory, not a file", field, value)
	}
...
func (c *Potatoes) GetDir(field string) (string, error) {
	value := viper.GetString(field)
	if value == "" {
		return "", nil
	}
...
func (c *Potatoes) GetURL(field string) (*url.URL, error) {
	value := viper.GetString(field)
	if value == "" {
		return nil, nil
	}
...
func (c *Potatoes) GetSSLMode(field string) (string, error) {
	value := viper.GetString(field)
	switch value {
...
func (c *Potatoes) GetContainerPlacementStrategy(field string) (string, error) {
	value := viper.GetString(field)
	switch value {
