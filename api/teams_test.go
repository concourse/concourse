...
var _ = Describe("Teams API", func() {
	...
	Describe("PUT /api/vi/teams/:team_name/rename", func() {
		var request *http.Request
		var response *http.Response
		...
		Context("when the requester is authenticated for the right team (admin team)", func() {
			JustBeforeEach(func() {
				path := fmt.Sprintf("%s/api/v1/teams/%s/rename", server.URL, teamName)
				var err error
				request, err = http.NewRequest("PUT", path, bytes.NewBufferString(`{"name":"some-new-name"}`))
				Expect(err).NotTo(HaveOccurred())
				response, err = client.Do(request)
				Expect(err).NotTo(HaveOccurred())
			})

			Context("when a team with the new name already exists", func() {
				BeforeEach(func() {
					authValidator.IsAuthenticatedReturns(true)
					userContextReader.GetTeamReturns(atc.DefaultTeamName, true, true)
					newSavedTeam := db.SavedTeam{
						ID: 2,
						Team: db.Team{
							Name: "some-new-name",
						},
					}
					teamDB.GetTeamReturns(newSavedTeam, true, nil)
				})

				It("fails to rename the team", func() {
					Expect(response.StatusCode).Should(Equal(http.StatusConflict))
				})
			})

			Context("when a team with the new name doesn't exist", func() {
				BeforeEach(func() {
					authValidator.IsAuthenticatedReturns(true)
					userContextReader.GetTeamReturns(atc.DefaultTeamName, true, true)
					var getTeamCalls = 0
					teamDB.GetTeamStub = func() (db.SavedTeam, bool, error) {
						switch getTeamCalls {
						case 0:
							getTeamCalls++
					...
					teamDB.UpdateNameReturns(newSavedTeam, nil)
				})
				...
				It("renames the team to the name provided", func() {
					...
					Expect(teamDB.UpdateNameCallCount()).To(Equal(1))
					Expect(teamDB.UpdateNameArgsForCall(0)).To(Equal("some-new-name"))
				})
			})
		})
	})
