...
type FakeImageResourceFetcher struct {
	FetchStub        func(logger lager.Logger, resourceUser dbng.ResourceUser, signals <-chan os.Signal, imageResourceType string, imageResourceSource atc.Source, id worker.Identifier, metadata worker.Metadata, tags atc.Tags, teamID int, customTypes dbng.ResourceTypes, imageFetchingDelegate worker.ImageFetchingDelegate, privileged bool) (worker.Volume, io.ReadCloser, atc.Version, error)
...
func (fake *FakeImageResourceFetcher) FetchArgsForCall(i int) (lager.Logger, dbng.ResourceUser, <-chan os.Signal, string, atc.Source, worker.Identifier, worker.Metadata, atc.Tags, int, dbng.ResourceTypes, worker.ImageFetchingDelegate, bool) {
	fake.fetchMutex.RLock()
	defer fake.fetchMutex.RUnlock()
