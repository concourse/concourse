...
func (s *SpecSuite) TestContainerSpecValidations() {
	for _, tc := range []struct {
		...
		s.T().Run(tc.desc, func(t *testing.T) {
			_, err := spec.OciSpec(spec.DefaultInitBinPath, specs.LinuxSeccomp{}, specs.Hooks{}, tc.spec, dummyMaxUid, dummyMaxGid)
			s.Error(err)
		})
	}
}
...
func (s *SpecSuite) TestContainerSpec() {
	...
	for _, tc := range []struct {
		...
		s.T().Run(tc.desc, func(t *testing.T) {
			actual, err := spec.OciSpec(spec.DefaultInitBinPath, specs.LinuxSeccomp{}, specs.Hooks{}, tc.gdn, dummyMaxUid, dummyMaxGid)
			s.NoError(err)
