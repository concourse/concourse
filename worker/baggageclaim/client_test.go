...
var _ = Describe("Baggageclaim Client", func() {
	var (
	...
	mockUnknownErrorResponse := func(method string, endpoint string, message string, status int) {
		bcServer.AppendHandlers(
			ghttp.CombineHandlers(
				ghttp.VerifyRequest(method, endpoint),
				ghttp.RespondWith(status, message),
			),
		)
	}
	...
	Describe("Looking up a volume by handle", func() {
		...
		Context("when an unknown error occurs", func() {
			It("returns HTTP status code and the body", func() {
				mockUnknownErrorResponse("GET", "/volumes/some-handle", "lost baggage", http.StatusInternalServerError)
				foundVolume, found, err := bcClient.LookupVolume(context.Background(), "some-handle")
				Expect(foundVolume).To(BeNil())
				Expect(found).To(BeFalse())
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(Equal("unexpected baggageclaim error response: 500 Internal Server Error: lost baggage"))
			})
		})
	})
	...
	Describe("Listing volumes", func() {
		...
		Context("when an unknown error occurs", func() {
			It("returns HTTP status code and the body", func() {
				mockUnknownErrorResponse("GET", "/volumes", "lost baggage", http.StatusInternalServerError)
				volumes, err := bcClient.ListVolumes(context.Background(), baggageclaim.VolumeProperties{})
				Expect(volumes).To(BeNil())
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(Equal("unexpected baggageclaim error response: 500 Internal Server Error: lost baggage"))
	...
	Describe("Destroying volumes", func() {
		...
		Context("when no volumes are destroyed", func() {
			...
			It("it returns an error", func() {
				mockErrorResponse("DELETE", "/volumes/destroy", "baggage not destroyed", http.StatusInternalServerError)

				err := bcClient.DestroyVolumes(context.Background(), handles)
				Expect(err).To(HaveOccurred())
				Expect(err).To(MatchError(client.ErrVolumeDeletion))
			})
		})
	})
	...
	Describe("Creating volumes", func() {
		...
		Context("when an unknown error occurs", func() {
			It("returns HTTP status code and the body", func() {
				...
				mockUnknownErrorResponse("GET", "/volumes-async/some-handle", "lost baggage", http.StatusInternalServerError)
				bcServer.AppendHandlers(
					ghttp.CombineHandlers(
						ghttp.VerifyRequest("DELETE", "/volumes-async/some-handle"),
						ghttp.RespondWith(http.StatusNoContent, ""),
	...
	Describe("Stream in a volume", func() {
		...
		Context("when an unknown error occurs", func() {
			It("returns HTTP status code and the body", func() {
				mockUnknownErrorResponse("PUT", "/volumes/some-handle/stream-in", "lost baggage", http.StatusInternalServerError)
				err := vol.StreamIn(context.TODO(), "./some/path/", baggageclaim.GzipEncoding, 0, strings.NewReader("even more tar"))
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(Equal("unexpected baggageclaim error response: 500 Internal Server Error: lost baggage"))
			})
	...
	Describe("Stream out a volume", func() {
		...
		Context("when an unknown error occurs", func() {
			It("returns HTTP status and the body", func() {
				mockUnknownErrorResponse("PUT", "/volumes/some-handle/stream-out", "lost baggage", http.StatusInternalServerError)
				_, err := vol.StreamOut(context.TODO(), "./some/path/", baggageclaim.GzipEncoding)
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(Equal("unexpected baggageclaim error response: 500 Internal Server Error: lost baggage"))
			})
	...
	Describe("Setting property on a volume", func() {
		...
		Context("when an unknown error occurs", func() {
			It("returns HTTP status and the body", func() {
				mockUnknownErrorResponse("PUT", "/volumes/some-handle/properties/key", "lost baggage", http.StatusInternalServerError)
				err := vol.SetProperty(context.Background(), "key", "value")
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(Equal("unexpected baggageclaim error response: 500 Internal Server Error: lost baggage"))
			})
	...
	Describe("Get p2p stream-in url", func() {
		...
		Context("when error occurs", func() {
			It("returns API error message", func() {
				...
				url, err := vol.GetStreamInP2pUrl(context.TODO(), "some-path")
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(Equal("failed to get p2p url"))
				Expect(url).To(BeEmpty())
			})
		})

		Context("when unknown error occurs", func() {
			It("returns HTTP status and the body", func() {
				mockUnknownErrorResponse("GET", "/p2p-url", "failed to get p2p url", http.StatusInternalServerError)
				url, err := vol.GetStreamInP2pUrl(context.TODO(), "some-path")
				Expect(err).To(HaveOccurred())
				Expect(err.Error()).To(Equal("unexpected baggageclaim error response: 500 Internal Server Error: failed to get p2p url"))
				Expect(url).To(BeEmpty())
			})
		})
	})
