...
var _ = Describe("Filesystem", func() {
	...
	Describe("Volume Lifecycle", func() {
		...
		It("cleans up if creating a sub-volume errors", func() {
			...
			logs := logger.Logs()
			Expect(len(logs)).To(Equal(1), "should log the Driver.DestroyVolume() error")
			Expect(logs[0].Message).To(Equal("fs.filesystem.error-cleaning-up-volume-after-fail-cow-creation"))
			Expect(logs[0].LogLevel).To(Equal(lager.ERROR))
			Expect(logs[0].Data["error"]).To(Equal("driver-destroy-error"))
		})
	})
})
