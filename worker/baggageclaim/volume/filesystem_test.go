...
var _ = Describe("Filesystem", func() {
	...
	It("initializes the init/live/dead directories", func() {
		Expect(path.Join(parentDir, "init")).To(BeADirectory())
		Expect(path.Join(parentDir, "live")).To(BeADirectory())
		Expect(path.Join(parentDir, "dead")).To(BeADirectory())
	})
	...
	Describe("NewVolume", func() {
		...
		It("destroys the volume if there is an error", func() {
			...
			Expect(path.Join(parentDir, "init", "some-volume")).ToNot(BeADirectory())
	...
	Describe("CleanupOrphanedEntries", func() {
		It("destroys stale dead/ volume entries", func() {
			...
			Expect(os.Mkdir(path.Join(parentDir, "dead/dead-vol-1"), 0755)).To(Succeed())
			Expect(os.Mkdir(path.Join(parentDir, "dead/dead-vol-2"), 0755)).To(Succeed())

			err := fs.CleanupOrphanedEntries()
			Expect(err).ToNot(HaveOccurred())

			// Dead dirs should be removed
			Expect(path.Join(parentDir, "dead/dead-vol-1")).ToNot(BeADirectory())
			Expect(path.Join(parentDir, "dead/dead-vol-2")).ToNot(BeADirectory())
		...
		It("collects live and init handles and passes them to RemoveOrphanedResources", func() {
			...
			Expect(os.Mkdir(path.Join(parentDir, "live/live-vol-1"), 0755)).To(Succeed())
			Expect(os.Mkdir(path.Join(parentDir, "live/live-vol-2"), 0755)).To(Succeed())
			Expect(os.Mkdir(path.Join(parentDir, "init/init-vol-1"), 0755)).To(Succeed())
		...
		It("logs errors from dead volume cleanup but continues", func() {
			...
			Expect(os.Mkdir(path.Join(parentDir, "dead/dead-vol"), 0755)).To(Succeed())

			err := fs.CleanupOrphanedEntries()
			Expect(err).ToNot(HaveOccurred())

			// Dead dir should still be cleaned up (os.RemoveAll after driver call)
			Expect(path.Join(parentDir, "dead/dead-vol")).ToNot(BeADirectory())
		})
