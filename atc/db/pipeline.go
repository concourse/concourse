...
func (p *pipeline) CausalityV2(resourceID int, versionFilter atc.Version) (atc.CausalityResourceVersion, error) {
	...
	fmt.Println("figuring out build ids")
	rows, err := p.conn.Query(`
	...
	fmt.Println("constructing builds")
	for rows.Next() {
		...
		if _, found := builds[buildID]; !found {
			...
			// 	jobs[jobID] = atc.CausalityJob{
			// 		ID:     jobID,
			// 		Name:   jobName,
			// 		Builds: []atc.CausalityBuild{b},
			// 	}
			// } else {
	...
	for rows.Next() {
		...
		if rv, found := resourceVersions[rcvID]; !found {
			...
			rv.InputTo = append(rv.InputTo, builds[bID])
		}
	}
	fmt.Println(resourceVersions)

	// do the same thing but with outputs. This *should* complete the tree
	fmt.Println("querying outputs")
