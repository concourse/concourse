...
func (c *component) IntervalElapsed() bool {
	...
	// Only recompute drift if enough time has passed or first call
	if now.Sub(c.lastDriftTime) > c.driftCacheTTL || c.lastDriftTime.IsZero() {
		c.lastDrift = c.computeDrift()
		c.lastDriftTime = now
	}

	interval := c.interval
	if interval+c.lastDrift > 0 {
		interval += c.lastDrift
	}

	return now.After(c.lastRan.Add(interval))
}
...
// - Calls to NumGoroutine() can be expensive
// - Results are cached via IntervalElapsed() to reduce overhead
// - Cache invalidation happens based on driftCacheTTL (typically 100ms)
//
...
func (c *component) computeDrift() time.Duration {
	...
	currentGoroutines := c.goRoutineCounter.NumGoroutine()
	relativeLoad := float64(currentGoroutines) / float64(c.numGoroutineThreshold)

	// relativeLoad = 0 => drift = -1 second
	// relativeLoad = 1 => drift = 0 second
	// relativeLoad = 2 => drift = 1 second
	// etc.
	drift := time.Duration((relativeLoad - 1) * float64(time.Second))

	// Cap the maximum drift
	if drift > maxDrift {
		drift = maxDrift
	}
