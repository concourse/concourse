...
func (self *migrations) Down_1537546150() error {
	...
	_, err = tx.Exec(`CREATE TABLE build_inputs (
      build_id integer,
	...
	_, err = tx.Exec(`CREATE INDEX build_inputs_build_id_idx ON build_inputs USING btree (build_id)`)
	if err != nil {
		tx.Rollback()
		return err
	}

	_, err = tx.Exec(`CREATE INDEX build_inputs_build_id_versioned_resource_id ON build_inputs USING btree (build_id, versioned_resource_id)`)
	if err != nil {
		tx.Rollback()
		return err
	}

	_, err = tx.Exec(`CREATE INDEX build_inputs_versioned_resource_id_idx ON build_inputs USING btree (versioned_resource_id)`)
	if err != nil {
		tx.Rollback()
		return err
	}

	_, err = tx.Exec(`ALTER TABLE ONLY build_inputs
	...
	_, err = tx.Exec(`CREATE TABLE build_outputs (
      build_id integer,
      versioned_resource_id integer
  )`)
	if err != nil {
	...
	_, err = tx.Exec(`CREATE INDEX build_outputs_build_id_idx ON build_outputs USING btree (build_id)`)
	if err != nil {
		tx.Rollback()
		return err
	}

	_, err = tx.Exec(`CREATE INDEX build_outputs_build_id_versioned_resource_id ON build_outputs USING btree (build_id, versioned_resource_id)`)
	if err != nil {
		tx.Rollback()
		return err
	}

	_, err = tx.Exec(`CREATE INDEX build_outputs_versioned_resource_id_idx ON build_outputs USING btree (versioned_resource_id)`)
	if err != nil {
		tx.Rollback()
		return err
	}

	_, err = tx.Exec(`ALTER TABLE ONLY build_outputs
