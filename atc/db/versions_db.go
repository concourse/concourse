...
func (versions VersionsDB) VersionExists(ctx context.Context, resourceID int, versionSHA256 ResourceVersion) (bool, error) {
	column := "version_sha256"
	if len(versionSHA256) < sha256.Size*2 {
		column = "version_md5"
	}
...
func (versions VersionsDB) FindVersionOfResource(ctx context.Context, resourceID int, v atc.Version) (ResourceVersion, bool, error) {
	...
	err = psql.Select("COALESCE(rcv.version_md5, rcv.version_sha256)").
...
func (versions VersionsDB) NextEveryVersion(ctx context.Context, jobID int, resourceID int) (ResourceVersion, bool, bool, error) {
	...
	err = tx.QueryRowContext(ctx, `
		...
		CROSS JOIN LATERAL (
			...
			AND i.version_digest IN (rcv.version_md5, rcv.version_sha256)
			LIMIT 1
		) AS inputs
	...
	rows, err := psql.Select("COALESCE(rcv.version_md5, rcv.version_sha256)").
		From("resource_config_versions rcv").
		Where(sq.Expr("rcv.resource_config_scope_id = (SELECT resource_config_scope_id FROM resources WHERE id = ?)", resourceID)).
		Where(sq.Expr("NOT EXISTS (SELECT 1 FROM resource_disabled_versions WHERE resource_id = ? AND version_digest IN (rcv.version_md5, rcv.version_sha256))", resourceID)).
...
func (versions VersionsDB) latestVersionOfResource(ctx context.Context, tx Tx, resourceID int) (ResourceVersion, bool, error) {
	...
	err = psql.Select("COALESCE(rcv.version_md5, rcv.version_sha256)").
		...
		Where(`NOT EXISTS (
				...
				WHERE rdv.resource_id = ?
				AND rdv.version_digest IN (rcv.version_md5, rcv.version_sha256)
			)`, resourceID).
		...
		WITH builds_to_migrate AS (
			...
			INSERT INTO successful_build_outputs (
				...
				JOIN (
					...
					FROM (
						(
							SELECT build_id, resource_id, version_digest
