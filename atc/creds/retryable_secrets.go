...
func (rs RetryableSecrets) Get(secretPath string, params SecretLookupParams) (any, *time.Time, bool, error) {
	r := &retryhttp.DefaultRetryer{}
	for i := 0; i < rs.retryConfig.Attempts-1; i++ {
		result, expiration, exists, err := rs.secrets.Get(secretPath, params)
...
func (rs RetryableSecrets) NewSecretLookupPaths(params SecretLookupParams, allowRootPath bool) []SecretLookupPath {
	return rs.secrets.NewSecretLookupPaths(params, allowRootPath)
}
