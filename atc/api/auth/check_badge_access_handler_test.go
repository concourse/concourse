...
var _ = Describe("CheckBadgeAccessHandler", func() {
	var (
	...
	BeforeEach(func() {
		...
		defaultBadgeVisibilityFactory.NewDefaultBadgeVisibilityReturns(auth.NewDefaultBadgeVisibility("pipeline-visibility"))

		pipeline = new(dbfakes.FakePipeline)

		handlerFactory := auth.NewCheckBadgeAccessHandlerFactory(teamFactory, defaultBadgeVisibilityFactory)
		fakeAccessor = new(accessorfakes.FakeAccessFactory)
		fakeaccess = new(accessorfakes.FakeAccess)

		delegate = &pipelineDelegateHandler{}
		checkBadgeAccessHandler := handlerFactory.HandlerFor(delegate, auth.UnauthorizedRejector{})
		handler = accessor.NewHandler(checkBadgeAccessHandler, fakeAccessor, "some-action", new(auditorfakes.FakeAuditor))
	})
	...
	Context("when pipeline exists", func() {
		...
		Context("when pipeline is private", func() {
			...
			Context("and unauthorized", func() {
				...
				Context("and badges are default public", func() {
					BeforeEach(func() {
						fakeaccess.IsAuthenticatedReturns(false)
						defaultBadgeVisibilityFactory.NewDefaultBadgeVisibilityReturns(auth.NewDefaultBadgeVisibility("public"))
					})
