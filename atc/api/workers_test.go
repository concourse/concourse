...
var _ = Describe("Workers API", func() {
	...
	Describe("PUT /api/v1/workers/:worker_name/overloaded", func() {
		var (
			response   *http.Response
			workerName string
			fakeWorker *dbfakes.FakeWorker
		...
		JustBeforeEach(func() {
			req, err := http.NewRequest("PUT", server.URL+"/api/v1/workers/"+workerName+"/overloaded", strings.NewReader("true"))
			Expect(err).NotTo(HaveOccurred())
		...
		BeforeEach(func() {
			...
			fakeWorker.NameReturns(workerName)
			fakeWorker.TeamNameReturns("some-team")
			fakeWorker.OverloadedReturns(true)
		...
		Context("when the request is authenticated as system", func() {
			...
			It("sees if the worker exists and sets the overloaded status", func() {
				Expect(dbWorkerFactory.GetWorkerCallCount()).To(Equal(1))
				Expect(dbWorkerFactory.GetWorkerArgsForCall(0)).To(Equal(workerName))
				Expect(fakeWorker.SetOverloadedCallCount()).To(Equal(1))
				Expect(fakeWorker.SetOverloadedArgsForCall(0)).To(BeTrue())
			})

			Context("when setting the overloaded status fails", func() {
				var returnedErr error

				BeforeEach(func() {
					returnedErr = errors.New("some-error")
					fakeWorker.SetOverloadedReturns(returnedErr)
				})
