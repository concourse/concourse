...
constructResourceVersion parentId downstream rv =
    let
        ...
        downstreamChildren =
            List.foldl (recurseChild True) { nodes = [], edges = [] } rv.inputTo

        upstreamChildren =
            List.foldl (recurseChild False) { nodes = [], edges = [] } rv.outputOf
    in
    { nodes =
        Node nodeId { nodeType = ResourceVersionNode rv.resourceName rv.version }
            :: downstreamChildren.nodes
            ++ upstreamChildren.nodes
    , edges =
        edge
            :: downstreamChildren.edges
            ++ upstreamChildren.edges
    }
...
constructBuild parentId downstream (Concourse.CausalityBuildVariant b) =
    let
        ...
        downstreamChildren =
            List.foldl (recurseChild True) { nodes = [], edges = [] } b.outputs

        upstreamChildren =
            List.foldl (recurseChild False) { nodes = [], edges = [] } b.inputs
    in
    { nodes =
