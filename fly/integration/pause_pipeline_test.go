...
var _ = Describe("Fly CLI", func() {
	Describe("pause-pipeline", func() {
		Context("when the pipeline name is specified", func() {
			...
			BeforeEach(func() {
				path, err = routes.Router().CreatePathForRoute(atc.PausePipeline, map[string]string{"pipeline_name": "awesome-pipeline", "team_name": teamName})
		...
		Context("when the --all flag is passed", func() {
			...
			BeforeEach(func() {
				somePath, err = routes.Router().CreatePathForRoute(atc.PausePipeline, map[string]string{"pipeline_name": "awesome-pipeline", "team_name": teamName})
				Expect(err).NotTo(HaveOccurred())

				someOtherPath, err = routes.Router().CreatePathForRoute(atc.PausePipeline, map[string]string{"pipeline_name": "more-awesome-pipeline", "team_name": teamName})
				Expect(err).NotTo(HaveOccurred())
