	...
	Describe("Put", func() {
		...
		Context("when the tracker can initialize the resource", func() {
			...
			It("initializes the resource with the correct type, session, and sources", func() {
				...
				_, sm, sid, typ, tags, sources, _ := fakeTracker.InitWithSourcesArgsForCall(0)
			...
			Context("before initializing the resource", func() {
				...
				BeforeEach(func() {
					...
					fakeTracker.InitWithSourcesStub = func(lager.Logger, resource.Metadata, resource.Session, resource.ResourceType, atc.Tags, map[string]resource.ArtifactSource, atc.ResourceTypes) (resource.Resource, []string, error) {
