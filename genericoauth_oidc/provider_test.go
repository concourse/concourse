...
var _ = Describe("Generic OAuth OIDC Provider", func() {
	Describe("Provider Constructor", func() {
		var (
			authConfig          *genericoauth_oidc.GenericOAuthOIDCConfig
			redirectURI         string
			goaProvider         provider.Provider
			state               string
			found               bool
			genericOIDCTeamProvider genericoauth_oidc.GenericOIDCTeamProvider
		)
		...
		JustBeforeEach(func() {
			genericOIDCTeamProvider = genericoauth_oidc.GenericOIDCTeamProvider{}
			goaProvider, found = genericOIDCTeamProvider.ProviderConstructor(authConfig, redirectURI)
		...
		It("enforces user authorization", func() {
			c := &oauth2.Config{}
			httpClient := c.Client(oauth2.NoContext, nil)
			verifyResult, err := goaProvider.Verify(lagertest.NewTestLogger("test"), httpClient)
	...
	Describe("AuthMethod", func() {
		...
		BeforeEach(func() {
			authConfig = &genericoauth_oidc.GenericOAuthOIDCConfig{DisplayName: "duck-song"}
		...
		It("creates path for route", func() {
			Expect(authMethod).To(Equal(provider.AuthMethod{
				...
				AuthURL:     "http://bum-bum-bum.com/auth/oauth_oidc?team_name=dudududum",
			}))
		})
	})
})
