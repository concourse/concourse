...
This release adds a new way of configuring jobs called @secref{build-plans}.
...
@itemlist[
  @item{
    ...
    @secref{build-plans}. Jobs no longer need to take the form of inputs to a
    build to outputs (although this is still possible). Jobs can now run steps
    in parallel, aggregate inputs together, and push to outputs in the middle
    of a build. The applications and possibilities are too numerous to list. So
    ...
    For more information on how you can start, see @secref{build-plans}. We've
  ...
  @item{
    ...
    to @secref{tasks} to disambiguate from builds of a job. Jobs have builds,
    and builds are the result of running tasks and resource actions.
  }
